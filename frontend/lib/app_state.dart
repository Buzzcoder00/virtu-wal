...
class AppState extends ChangeNotifier {
  ...
  Future<void> updateProfilePicture() async {
    ...
    try {
      ...
      final File localImage =
          await File(image.path).copy('$path/profile_${_user!.uid}.jpg');

      localProfilePath = localImage.path;
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(
          'localProfilePath_${_user!.uid}', localProfilePath!);
  ...
  Future<void> _loadFromCache() async {
    try {
      ...
      if (transactionsJson != null) {
        ...
        transactions.clear();
        transactions
            .addAll(decoded.map((item) => WalletTransaction.fromJson(item)));
      }
  ...
  Future<void> _saveToCache() async {
    ...
    await prefs.setString(
        'tempWallets', jsonEncode(tempWallets.map((w) => w.toJson()).toList()));
        ...
        if (data != null) {
          ...
          if (qrStatus == 'completed') {
            ...
            final txnId = data['transactionId'] ??
                'CLD-${DateTime.now().millisecondsSinceEpoch}';
  ...
  void _processIncomingPayment(double amount, String senderName, String txnId) {
    ...
    tempWallets = tempWallets
        .map((wallet) => wallet.id == activeQR!.walletId
            ? wallet.copyWith(balance: wallet.balance + amount)
            : wallet)
        .toList();
    activeQR =
        activeQR!.copyWith(currentAmount: activeQR!.currentAmount + amount);
    if (activeQR!.limitType == LimitType.amount &&
        (activeQR!.amountLimit ?? 0) <= activeQR!.currentAmount) {
  ...
  void deleteWallet(String walletId) {
    ...
    if (tempWallets[walletIndex].balance > 0)
      transferWalletToRealAccount(walletId);
  ...
  Future<void> generateQR(
      LimitType limitType, double value, String walletId) async {
  ...
  Future<void> notifyScanning(String qrId) async {
    try {
      ...
      debugPrint('Update status to scanning failed: $e');
    }
  }
  ...
  Future<void> scannerPayment(String qrId, double amount) async {
    ...
    try {
      final docRef =
          FirebaseFirestore.instance.collection('payments').doc(qrId);
      ...
      if (data['expiresAt'] != null) {
        ...
        if (DateTime.now().isAfter(expiry)) {
          _addFailedTransaction('QR Code Expired', amount,
      ...
      await FirebaseFirestore.instance
          .collection('users')
          .doc(_user!.uid)
          .update({'balance': realAccountBalance});
  ...
  void _addFailedTransaction(String reason, double amount, {String? receiver}) {
    transactions.insert(
        ...
        WalletTransaction(
          ...
          walletId: 'EXTERNAL',
          walletName:
              receiver != null ? 'Payment to $receiver' : 'External Payment',
  ...
  void transferWalletToRealAccount(String walletId, {bool auto = false}) async {
    ...
    if (auto) {
      _speak(
          "Limit reached. ${transferAmount.toInt()} rupees auto transferred to main account.");
    }
    ...
    transactions.insert(
        ...
        WalletTransaction(
            ...
            type: auto
                ? TransactionType.autoTransferred
  ...
  List<TempWallet> get activeWallets =>
      tempWallets.where((wallet) => !wallet.isExpired).toList();
  List<TempWallet> get expiredWallets =>
      tempWallets.where((wallet) => wallet.isExpired).toList();
  ...
  void simulatePayment(double amount) {
    if (activeQR == null) return;
    _processIncomingPayment(
        amount, 'Simulator', 'SIM-${DateTime.now().millisecondsSinceEpoch}');
  }

  double getTotalTempBalance() => tempWallets
      .where((wallet) => !wallet.isExpired)
      .fold(0, (sum, wallet) => sum + wallet.balance);

  TempWallet? getCurrentWallet() {
    if (selectedWalletId == null) return null;
    return tempWallets.firstWhere((w) => w.id == selectedWalletId,
        orElse: () => tempWallets.first);
  }
